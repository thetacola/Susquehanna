...
\title{Susquehanna User Manual - v0.3.0 Colliersville}
\date{\today\\Revision 7}
\author{thetacola}
\usepackage{graphicx}
...
\begin{document}
	...
	\begin{tcolorbox}[width=1\textwidth]
		\textsc{For developers:} Language files are stored in your home directory. On Linux, this can be found at \emph{\textasciitilde/Susquehanna/}, and on Windows it can be found at \emph{C:{\textbackslash}Users{\textbackslash}\{username\}{\textbackslash}Susquehanna} on a typical configuration. The directory is created in the \emph{user.home} folder provided by Java, so if the folder cannot be located in these places, check your \emph{user.home}. This location of your \emph{user.home} can be found by checking the debug output in the Info book. There are three types of files that can be found in this directory, those being \emph{.language}, \emph{.xml}, and \emph{.meta}. \emph{.language} files are those created by versions of Susquehanna before v0.3.0. These are automatically converted on startup, and are moved to backup files just in case something in the conversion goes awry. \emph{.xml} is what languages are stored in for modern versions of Susquehanna (v0.3.0+). \emph{.meta} files store the metadata that appears in the application before they are fully loaded, preventing the need for these files to be fully parsed on application start. Other files, such as phonological systems, glossing systems, and logs can also be found in this directory.
	\end{tcolorbox}
